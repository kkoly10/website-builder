...
export function emailWrap(
  ...
  //     Outlook web. Gmail strips media queries on most renderers, so
  //     the light variant is what Gmail dark-theme users see — Gmail's
  //     own color inversion handles legibility on its own backdrop.
  //
  ...
  //     pass; ogsc marks color-shifted elements and ogsb marks
  //     background-shifted ones. We emit the full rule set under BOTH
  //     prefixes because Outlook's shift selection isn't predictable —
  //     missing one path leaves orphan-styled elements (e.g., the logo
  //     swap previously fired under ogsc but not ogsb, so card-header
  //     background flipped without the logo swapping with it).
  //
  // All three paths share the same target palette, generated by
  // emitDarkRules so adding a new rule only requires editing one
  // place instead of three. Falls back to the light variant when no
  // path matches (a small minority of clients that strip <style>
  // tags entirely).
  function emitDarkRules(prefix: string): string {
    const p = prefix ? `${prefix} ` : "";
    return `
    ${p}body, ${p}.body-bg { background:#0f0f0f !important; }
    ${p}.card { background:#1a1a1a !important; border-color:#2a2a2a !important; }
    ${p}.card-header { background:#1a1a1a !important; border-bottom-color:#2a2a2a !important; }
    ${p}.logo-light { display:none !important; max-height:0 !important; max-width:0 !important; overflow:hidden !important; visibility:hidden !important; }
    ${p}.logo-dark { display:inline-block !important; max-height:none !important; max-width:280px !important; overflow:visible !important; visibility:visible !important; }
    ${p}.card-body { color:#e8e8e8 !important; }
    ${p}.card-body p, ${p}.card-body td { color:#cccccc !important; }
    ${p}.card-body h1, ${p}.card-body h2 { color:#ffffff !important; }
    ${p}.card-body strong { color:#ffffff !important; }
    ${p}.card-footer { background:#141414 !important; border-color:#2a2a2a !important; }
    ${p}.footer-text, ${p}.footer-text a { color:#888888 !important; }
    ${p}.callout td { background:#141414 !important; border-left-color:#ffffff !important; }
    ${p}.callout td p { color:#cccccc !important; }
    ${p}.callout td p:first-child { color:#888888 !important; }
    ${p}.admin-badge td { background:#2a2a2a !important; }
    ${p}.admin-badge td p { color:#aaaaaa !important; }
    ${p}.booked-box td { background:#222222 !important; }
    ${p}.booked-box td p { color:#e8e8e8 !important; }
    ${p}.booked-box td p:first-child { color:#888888 !important; }
    ${p}.cal-btn { background:#2a2a2a !important; border-color:#444444 !important; color:#e8e8e8 !important; }
    ${p}.row-label { color:#888888 !important; }
    ${p}.row-value { color:#e8e8e8 !important; }
    ${p}.row-value a { color:#7eb7ff !important; }
    ${p}.sig-name { color:#ffffff !important; }
    ${p}.sig-role { color:#999999 !important; }
    ${p}.body-link { color:#7eb7ff !important; }
    ${p}.cta-link { background:#ffffff !important; color:#111111 !important; }
  `;
  }
  const darkModeCss = `
    ...
    ${emitDarkRules("[data-ogsc]")}
    ${emitDarkRules("[data-ogsb]")}
  `;
    ...
    <td align="center">
      ...
      <table cellpadding="0" cellspacing="0" border="0" role="presentation" class="card" style="max-width:600px;width:100%;background:#ffffff;border:1px solid #e2e2e2">
        ...
        <!-- Header: bitmap logo (PNG rendered at 2x for retina) with
             ...
             Two img tags ship — a light variant (dark-on-transparent
             wordmark, default visible) and a dark variant (cream-on-
             transparent wordmark, default hidden via clipping rather
             than display:none). The hiding pattern matters: display:none
             gets stuck in legacy webmail clients (AOL, Yahoo Classic)
             that strip <style> tags, leaving the dark variant unable
             to ever appear. max-width:0 + max-height:0 + overflow:hidden
             + visibility:hidden achieves the same visual effect but is
             trivially undone by the dark-mode CSS overriding to non-zero
             dimensions. Both PNGs are 560×100 native, rendered at 280×50
             so the source is a 2× retina asset. Both carry alt text so
             screen readers and image-blocked clients still announce the
             brand in either mode. -->
        <tr>
          <td class="card-header" align="center" style="background:#ffffff;padding:28px 32px;border-bottom:1px solid #ececec">
            <a href="${SITE_URL}" style="text-decoration:none;display:block">
              <img class="logo-light" src="${SITE_URL}/brand/crecy-email-logo.png" alt="CrecyStudio" width="280" height="50" style="display:block;margin:0 auto;border:0;outline:none;text-decoration:none;height:auto;max-width:280px;width:100%;color:#111111;font-family:Arial,Helvetica,sans-serif;font-size:24px;font-weight:bold;line-height:50px;letter-spacing:-0.01em">
              <img class="logo-dark" src="${SITE_URL}/brand/crecy-email-logo-dark.png" alt="CrecyStudio" width="280" height="50" style="display:block;margin:0 auto;border:0;outline:none;text-decoration:none;height:auto;max-width:0;width:100%;color:#f8f1e8;font-family:Arial,Helvetica,sans-serif;font-size:24px;font-weight:bold;line-height:50px;letter-spacing:-0.01em;max-height:0;overflow:hidden;visibility:hidden">
            </a>
          </td>
        </tr>
