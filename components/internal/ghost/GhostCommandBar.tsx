    ...
    <section className="card" style={{ marginTop: 10 }}>
      <div className="cardInner">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", gap: 8 }}>
          <select className="input" value={lane} onChange={(e) => setLane(e.target.value as GhostLane)}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          <input className="input" placeholder="Ask Ghost (e.g., What is this project waiting on?)" value={question} onChange={(e) => setQuestion(e.target.value)} />
          <button className="btn btnPrimary" onClick={ask} disabled={busy}>{busy ? "Asking..." : "Ask"}</button>
        </div>
        {msg ? <p className="pDark" style={{ color: "#ef4444" }}>{msg}</p> : null}
