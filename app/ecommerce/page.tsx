  ...
  return (
    <main className="container" style={{ paddingBottom: 80 }}>
      <section className="section" style={{ padding: "80px 0 60px", textAlign: "center", maxWidth: 900, margin: "0 auto" }}>
        ...
        <p className="p" style={{ marginTop: 20, fontSize: 20 }}>From website setup and marketplace support to inventory storage and fulfillment, we help growing sellers stop doing everything themselves.</p>
      ...
      <section className="section" style={{ borderTop: "1px solid var(--stroke)", paddingTop: 56 }}>
        ...
        <div style={{ display: "grid", gap: 14 }}>{SERVICES.map((service) => <div className="card" key={service.title}><div className="cardInner" style={{ padding: 26 }}><h3 className="h3">{service.title}</h3><ul style={{ margin: "8px 0 0", color: "var(--muted)", lineHeight: 1.8 }}>{service.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul></div></div>)}</div>
      </section>
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        <div className="card" style={{ background: "var(--panel2)" }}>
          <div className="cardInner" style={{ padding: 30 }}>
            <div className="kicker">Why sellers work with us</div>
            <h2 className="h2" style={{ marginTop: 12 }}>Digital setup + operations support in one lane</h2>
            <p className="pDark">Most providers focus on just one side of your growth. We bridge setup and execution — website launch, listing support, inventory handling, and practical fulfillment coordination — so your operation is stable as sales increase.</p>
            <div className="grid2" style={{ marginTop: 12 }}>
              <div><strong>DIY to supported ops:</strong><p className="pDark">Start with one pain point (website, storage, or fulfillment) and expand as your order volume grows.</p></div>
              <div><strong>Operational continuity:</strong><p className="pDark">One team understands your seller workflow end-to-end, reducing handoff errors and delays.</p></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        ...
        <div className="grid2">{["Website Setup", "Storage", "Fulfillment", "Management Support"].map((item) => <div className="card" key={item}><div className="cardInner"><h3 className="h3">{item}</h3><p className="pDark">Starting at flexible levels based on scope, volume, and support model.</p></div></div>)}</div>
        <div style={{ marginTop: 18, textAlign: "center" }}><Link href="/ecommerce/pricing" className="btn btnGhost">View Full Pricing</Link></div>
      </section>

      <section className="section" style={{ paddingTop: 0 }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}><div className="kicker">Example use cases</div></div>
        <div className="grid2">
          {[ ["Website only", "Build your storefront and product pages while you manage fulfillment internally."], ["Storage only", "Keep overflow inventory organized with shelf/pallet options and monthly visibility."], ["Fulfillment only", "Get pick/pack and shipping support while keeping your current channels."], ["Full-service support", "Combine store setup, marketplace management, storage, and fulfillment into one plan."]].map(([title, copy]) => <div className="card" key={title}><div className="cardInner"><h3 className="h3">{title}</h3><p className="pDark">{copy}</p></div></div>) }
        </div>
      </section>
