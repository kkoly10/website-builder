...
const BUDGET_LABELS: Record<QuizBudget, string> = {
...
const CONTENT_LABELS: Record<QuizContent, string> = {
  ready: "Yes — copy and images are ready",
  some: "Partially — I have some content",
  "not-ready": "No — I will need help or time",
};
...
function recommendationFromQuiz(quiz: QuizState) {
  ...
  if (quiz.goal === "show-work") {
    return {
      ...
      bullets: [
        "Visual case-study structure",
        "SEO-friendly content blocks",
        "Lead capture across portfolio pages",
      ],
    };
  }
  return {
...
export default function BuildIntroPage() {
  ...
  }, [quiz, recommendation.intent, recommendation.websiteType]);
  ...
  return (
    <main className="container" style={{ maxWidth: 860, padding: "64px 0 96px" }}>
      ...
      <h1 className="h1" style={{ marginTop: 16 }}>
        Find your best website setup in under 60 seconds.
      </h1>
      <p className="p" style={{ marginTop: 12 }}>
        Answer a few quick questions and we&apos;ll guide you to the right build
        plan before you enter the full estimate flow.
      </p>
      ...
      <section className="panel" style={{ marginTop: 28 }}>
        <div className="panelHeader">
          ...
          <p className="pDark" style={{ marginTop: 8 }}>
            This helps us recommend the right website type, scope, and timeline.
          </p>
        </div>
        ...
        <div className="panelBody" style={{ display: "grid", gap: 16 }}>
          ...
          <div className="grid2">
            <div>
              ...
              <select
                ...
                value={quiz.pages}
                onChange={(e) =>
                  setQuiz((q) => ({ ...q, pages: e.target.value as QuizPages }))
                }
              >
            ...
            <div>
              ...
              <select
                ...
                value={quiz.timeline}
                onChange={(e) =>
                  setQuiz((q) => ({ ...q, timeline: e.target.value as QuizTimeline }))
                }
              >
          ...
          <div className="grid2">
            <div>
              <div className="fieldLabel">Is your content ready?</div>
              <select
                className="select"
                value={quiz.contentReady}
                onChange={(e) =>
                  setQuiz((q) => ({ ...q, contentReady: e.target.value as QuizContent }))
                }
              >
                {(Object.entries(CONTENT_LABELS) as [QuizContent, string][]).map(
                  ([val, label]) => (
            ...
            <div>
              <div className="fieldLabel">Rough budget in mind?</div>
              <select
                className="select"
                value={quiz.budget}
                onChange={(e) =>
                  setQuiz((q) => ({ ...q, budget: e.target.value as QuizBudget }))
                }
              >
                {(Object.entries(BUDGET_LABELS) as [QuizBudget, string][]).map(
                  ([val, label]) => (
                    <option key={val} value={val}>{label}</option>
                  )
                )}
      ...
      <section className="panel" style={{ marginTop: 20 }}>
        ...
        <div className="panelBody">
          <p className="p" style={{ marginBottom: 10 }}>
            <strong style={{ color: "var(--fg)" }}>{recommendation.headline}</strong>
          </p>
          ...
          {quiz.budget !== "not-sure" && (
            ...
            >
              Budget noted:{" "}
              <strong>{BUDGET_LABELS[quiz.budget]}</strong> — we&apos;ll factor
              this into your estimate.
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 20, flexWrap: "wrap" }}>
            ...
            <Link href="/systems" className="btn btnGhost">
              I need workflow systems instead
            </Link>
          </div>
        </div>
      </section>
