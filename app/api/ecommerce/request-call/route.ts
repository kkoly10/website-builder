...
export async function POST(req: NextRequest) {
  try {
    ...
    if (intakeErr) return NextResponse.json({ ok: false, error: intakeErr.message }, { status: 500 });
    if (!intake) return NextResponse.json({ ok: false, error: "E-commerce intake not found" }, { status: 404 });
    ...
    if (shouldUpdate) {
      ...
      if (createErr || !created?.id) {
        return NextResponse.json({ ok: false, error: createErr?.message || "Failed to save call request" }, { status: 500 });
      }
