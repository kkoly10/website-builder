...
function normalizeAdjustments(raw: any) {
  const src = toObj(raw);
  ...
  for (const key of ["discountPct", "discountAmount", "increaseAmount", "customTarget"]) {
    const v = src[key];
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pricingAdjustments = normalizeAdjustments(body.pricingAdjustments);
    ...
    admin.pricingAdjustmentsUpdatedAt = new Date().toISOString();
