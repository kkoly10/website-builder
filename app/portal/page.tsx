...
export default async function PortalPage() {
  ...
  const [{ data: quotes }, { data: opsIntakes }, { data: ecomByAuth }, { data: ecomByEmail }] =
    await Promise.all([
      ...
      supabaseAdmin
        .from("ops_intakes")
        .select("id, created_at, company_name, status, email, owner_email_norm")
        .eq("auth_user_id", user.id)
        .order("created_at", { ascending: false })
        .limit(25),
      supabaseAdmin
        ...
        <div className="card">
          <div className="cardInner">
            <div className="statRow" style={{ padding: 0, gap: 12 }}>
              ...
              <div className="stat">
                <div className="statNum">{ecomInProgress}</div>
                <div className="statLab">E-Commerce In Progress</div>
              </div>
            </div>
