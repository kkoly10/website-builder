...
function buildRecommendation(intake: any) {
  ...
  if (services.includes("Website only") || services.includes("Hosting / maintenance") || services.includes("Product listing help")) {
    setupFee += 700;
  }
  ...
  if (monthlyOrders > 1000) monthlyFee += 400;
  if (skuCount > 200) monthlyFee += 150;

  return {
    setupFee,
    monthlyFee,
    fulfillmentModel: services.includes("Fulfillment") ? "Hybrid per-order + storage support" : "Website + operations support",
    status: "draft",
    quoteJson: {
      source: "ecommerce-admin-baseline",
      recommended_modules: services,
      assumptions: { monthlyOrders, skuCount },
...
export default async function EcommerceAdminDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: intake }, { data: quote }, { data: call }, snapshot] = await Promise.all([
    ...
    getGhostProjectSnapshot("ecommerce", intakeId),
  ]);
  ...
  return (
    <section className="section" style={{ paddingTop: 0 }}>
      ...
      <div className="card">
        <div className="cardInner">
          <div className="kicker">
            <span className="kickerDot" /> E-Commerce Lead Workspace
          </div>
          ...
          <div className="pills" style={{ marginTop: 10 }}>
            <span className="pill">Intake Status: {statusLabel(intake.status, "new")}</span>
            <span className="pill">Call Status: {statusLabel(call?.status, "not requested")}</span>
            <span className="pill">Quote Status: {statusLabel(quote?.status, "not started")}</span>
      ...
      <div className="grid2" style={{ marginTop: 12 }}>
        ...
        <div className="card">
          <div className="cardInner" style={{ display: "grid", gap: 8 }}>
            <h2 className="h3">Latest call request</h2>
            <div className="pDark"><strong>Call Status:</strong> {statusLabel(call?.status, "not requested")}</div>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="cardInner" style={{ display: "grid", gap: 8 }}>
          <h2 className="h3">Latest quote summary</h2>
          <div className="pDark"><strong>Quote Status:</strong> {statusLabel(quote?.status, "not started")}</div>
      ...
      <details className="card" style={{ marginTop: 12 }}>
        <summary className="cardInner" style={{ cursor: "pointer", fontWeight: 700 }}>Raw data (debug)</summary>
        <div className="cardInner" style={{ paddingTop: 0 }}>
          <pre style={{ margin: 0, fontSize: 12, overflowX: "auto" }}>{JSON.stringify({ intake, call, quote }, null, 2)}</pre>
        </div>
      </details>
        </div>
        <GhostProjectSidebar lane="ecommerce" projectId={intakeId} snapshot={snapshot} />
      </div>
    </section>
  );
}
