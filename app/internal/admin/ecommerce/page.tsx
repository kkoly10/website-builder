...
type AnalyticsRow = { event: string; created_at: string | null };
...
export default async function EcommerceAdminPage({
  ...
  const lookbackIso = new Date(Date.now() - 30 * 86_400_000).toISOString();
  ...
  try {
    const { data: events } = await supabaseAdmin
      ...
      .in("event", [
        "ecom_intake_viewed",
        "ecom_intake_submit_succeeded",
        "ecom_call_booking_viewed",
        "ecom_call_booking_submit_succeeded",
        "ecom_quote_sent",
      ])
      .order("created_at", { ascending: false })
      .limit(2000);

    analyticsRows = (events ?? []) as AnalyticsRow[];
  } catch {
    analyticsRows = [];
  }

  const eventCount = (eventName: string) => analyticsRows.filter((row) => row.event === eventName).length;
  const funnel = {
    intakeViews: eventCount("ecom_intake_viewed"),
    intakeSubmits: eventCount("ecom_intake_submit_succeeded"),
    callViews: eventCount("ecom_call_booking_viewed"),
    callRequests: eventCount("ecom_call_booking_submit_succeeded"),
    quotesSent: eventCount("ecom_quote_sent"),
  };
  ...
  return (
    <section className="section" style={{ paddingTop: 0 }}>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="cardInner">
          <h2 className="h3">30-day funnel snapshot</h2>
          <div className="pills" style={{ marginTop: 8 }}>
            <span className="pill">Intake views: {funnel.intakeViews}</span>
            <span className="pill">Intake submits: {funnel.intakeSubmits}</span>
            <span className="pill">Call views: {funnel.callViews}</span>
            <span className="pill">Call requests: {funnel.callRequests}</span>
            <span className="pill">Quotes sent: {funnel.quotesSent}</span>
          </div>
          <div className="pills" style={{ marginTop: 8 }}>
            <span className="pill">Intake CVR: {pct(funnel.intakeSubmits, funnel.intakeViews)}</span>
            <span className="pill">Call-booking CVR: {pct(funnel.callRequests, funnel.callViews)}</span>
            <span className="pill">Submit → Sent: {pct(funnel.quotesSent, funnel.intakeSubmits)}</span>
          </div>
        </div>
      </div>
