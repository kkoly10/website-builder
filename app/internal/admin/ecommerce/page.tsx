...
export default async function EcommerceAdminPage() {
  ...
  return (
    <section className="section" style={{ paddingTop: 0 }}>
      <div className="card"><div className="cardInner"><div className="kicker"><span className="kickerDot" /> E-Commerce Pipeline</div><h1 className="h2">Seller intake leads</h1><p className="pDark">This table is separated from website quote and ops intake lanes.</p></div></div>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="cardInner" style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ textAlign: "left", borderBottom: "1px solid var(--stroke)" }}>
                <th style={{ padding: "10px 8px" }}>Business</th><th style={{ padding: "10px 8px" }}>Contact</th><th style={{ padding: "10px 8px" }}>Email</th><th style={{ padding: "10px 8px" }}>Channels</th><th style={{ padding: "10px 8px" }}>Services</th><th style={{ padding: "10px 8px" }}>Monthly Orders</th><th style={{ padding: "10px 8px" }}>Storage Type</th><th style={{ padding: "10px 8px" }}>Quote Status</th><th style={{ padding: "10px 8px" }}>Call Status</th><th style={{ padding: "10px 8px" }}>Created</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id} style={{ borderBottom: "1px solid var(--stroke)" }}>
                    ...
                    <td style={{ padding: "10px 8px" }}>{latestQuote?.status || "not started"}</td>
                    <td style={{ padding: "10px 8px" }}>{latestCall?.status || "not requested"}</td>
          ...
          {rows.length === 0 ? <p className="pDark" style={{ marginTop: 14 }}>No e-commerce intakes yet.</p> : null}
        </div>
      </div>
    </section>
  );
