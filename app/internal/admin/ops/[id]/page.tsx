...
export default async function OpsAdminDetailPage({
  ...
  if (!user) redirect(`/login?next=${encodeURIComponent(`/internal/admin/ops/${opsIntakeId}`)}`);

  const admin = await isAdminUser({ userId: user.id, email: user.email });
  if (!admin) redirect("/portal");

  const data = await getOpsWorkspaceBundle(opsIntakeId);
  if (!data) notFound();

  return <OpsProjectControlClient initialData={data} />;
}
