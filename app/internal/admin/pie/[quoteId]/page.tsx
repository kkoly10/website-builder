...
export default async function WebQuoteDetailPage(props: { params: ParamsPromise }) {
  ...
  const [{ data: quote }, { data: pieReports }, snapshot] = await Promise.all([
    ...
    getGhostProjectSnapshot("website", quoteId),
  ]);

  if (!quote) return <main className="container"><p>Quote not found.</p></main>;
  ...
  return (
    <section className="section" style={{ paddingTop: 0 }}>
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 360px", gap: 12, alignItems: "start" }}>
          ...
          <div className="card">
            <div className="cardInner">
              <div className="kicker">
                <span className="kickerDot" /> Web Design Quote
              </div>
              <h1 className="h2">
                ${quoteTotal.toLocaleString()} — {leadEmail}
              </h1>
          ...
          <div className="card">
            <div className="cardInner">
              ...
              {pieData ? (
                ...
                <p className="pDark" style={{ fontStyle: "italic" }}>
                  No PIE generated yet. Click the button above to analyze this lead.
                </p>
              )}
            </div>
          </div>
        ...
        <GhostProjectSidebar lane="website" projectId={quoteId} snapshot={snapshot} />
      </div>
    </section>
  );
}
