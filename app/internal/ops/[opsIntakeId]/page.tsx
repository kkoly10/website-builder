...
export default async function OpsIntakeDetailPage(props: { params: ParamsPromise }) {
  ...
  const [{ data: intake }, { data: calls }, { data: reports }, snapshot] = await Promise.all([
    ...
    getGhostProjectSnapshot("ops", opsIntakeId),
  ]);
  ...
  return (
    <section className="section" style={{ paddingTop: 0 }}>
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 360px", gap: 12, alignItems: "start" }}>
          ...
          <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12 }}>
            ...
            <div className="card">
              <div className="cardInner">
                ...
                {latestCall ? (
                  <div className="pDark">
                    <strong>Status:</strong> <span className="badge">{latestCall.status || "submitted"}</span>
                    <br />
            ...
            <div className="cardInner">
              ...
              {pieData ? (
                ...
                <p className="pDark" style={{ fontStyle: "italic" }}>
                  No PIE generated yet. Click the button above to analyze this workflow request.
                </p>
              )}
            </div>
          </div>
        ...
        <GhostProjectSidebar lane="ops" projectId={opsIntakeId} snapshot={snapshot} />
      </div>
    </section>
  );
}
