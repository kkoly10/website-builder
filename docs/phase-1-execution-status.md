...
| **Brand — official SVG logo** | [#94](https://github.com/kkoly10/website-builder/pull/94) | ✅ Merged | Replaced CSS text logo with `<img src="/brand/crecy-d1-horizontal-dark.svg">` in `BrandLogo.tsx`. Updated `app/icon.svg` to official app icon (dark rounded rect, lightning-bolt mark, red dot). All 13 brand SVG variants from the logo sheet published under `public/brand/`. Removed old CS monogram PNGs. CSS logo classes cleaned up. |
| **Brand — logo visibility fix** | [#95](https://github.com/kkoly10/website-builder/pull/95) | ✅ Merged | Logo fills were near-white (`#f8f1e8`) — invisible on the light paper nav. Switched to dark fills (`#1a1210`) and converted from `<img>` to inline SVG JSX so `var(--font-display)` resolves in the DOM and Sora renders correctly. Added `aria-hidden="true"` on the SVG (parent Link already has `aria-label`). |
| **Brand — CrecyStudio one word, Studio red** | [#96](https://github.com/kkoly10/website-builder/pull/96) | ✅ Merged | Rewritten `BrandLogo.tsx` as hand-crafted inline SVG: "CrecyStudio" as one word via `<tspan>` — "Crecy" dark (`#1a1210`), "Studio" red (`#c43e2b`), weight 300. Logo height lifted from 28→40px via CSS (`.brandLogo svg { height: 40px; width: auto }`). Responsive: 40→36→32px at 980px/360px breakpoints. Old unused PNGs removed. |
| **Brand — capitalize C and S** | [#97](https://github.com/kkoly10/website-builder/pull/97) | ✅ Merged | Single tspan text fix: `crecystudio` → `CrecyStudio`. |
...
| **Color + CTA bug pass** | [#101](https://github.com/kkoly10/website-builder/pull/101) | ✅ Merged | `home.module.css`: 5 `accent-soft` usages on dark backgrounds → `var(--accent)` (vivid red). Status doc logged PRs #94–#100. |
| **Color + CTA fixes round 2** | [#102](https://github.com/kkoly10/website-builder/pull/102) | ✅ Merged | `marketing.module.css`: `.darkStep` on dark process cards → `var(--accent)`. Homepage portal CTA `/portal` → `/demos/portal` (was 404). Stale Phase 1 comment removed. Portal hub CONTINUE button: was `/build/intro` for all quotes without workspace — now routes to `/book?quoteId=...&token=...` (view estimate) if `public_token` exists, or `/contact` if not. New `portal.common.viewEstimate` + `getInTouch` keys added to en/fr/es. |
